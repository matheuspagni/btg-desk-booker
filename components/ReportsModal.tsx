...
export default function ReportsModal({ isOpen, onClose }: ReportsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 max-w-4xl w-full h-[calc(100vh-2rem)] sm:h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-3 sm:p-6 border-b border-gray-200">
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-btg-blue-light bg-opacity-20 rounded-lg flex items-center justify-center">
              <svg className="w-4 h-4 sm:w-6 sm:h-6 text-btg-blue-deep" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </div>
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Relatórios</h2>
              <p className="text-xs sm:text-sm text-gray-600 hidden sm:block">Análises e estatísticas do sistema</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="border-b border-gray-200 px-4 sm:px-6 py-4 sm:py-5">
          <div className="flex items-center justify-center space-x-3 sm:space-x-4">
            <label className="text-sm text-gray-600 hidden sm:block">Período:</label>
            <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <button
              ...
              className="px-3 sm:px-4 py-2 bg-btg-blue-bright text-white rounded-lg hover:bg-btg-blue-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-1 sm:space-x-2 text-sm"
            >
              {loading ? (
                <>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-6 min-h-0" style={{ height: 'calc(100% - 120px)' }}>
          {activeTab === 'overview' && (
            <div className="space-y-4 sm:space-y-6">
              {loading ? (
                <div className="flex items-center justify-center py-8 sm:py-12">
                  <div className="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-btg-blue-bright"></div>
                  <span className="ml-3 text-sm sm:text-base text-gray-600">Carregando dados...</span>
                </div>
              ) : (
                <>

                  <div className="bg-gray-50 rounded-lg p-4 sm:p-6 border border-gray-200">
                    <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-3 sm:mb-4">Resumo Geral</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      <div className="bg-white rounded-lg p-3 sm:p-4 border border-gray-200">
                        <h4 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">Total de Reservas</h4>
                        <p className="text-2xl sm:text-3xl font-bold text-btg-blue-deep">
                      ...
                      <div className="bg-white rounded-lg p-3 sm:p-4 border border-gray-200">
                        <h4 className="font-medium text-gray-900 mb-2 text-sm sm:text-base">Ocupação Atual</h4>
          ...
          {activeTab === 'usage' && (
            <div className="space-y-4 sm:space-y-6">
              {loading ? (
                <div className="flex items-center justify-center py-8 sm:py-12">
                  <div className="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-btg-blue-bright"></div>
                  <span className="ml-3 text-sm sm:text-base text-gray-600">Carregando dados de uso...</span>
                </div>
              ) : (
                <>
                        ...
                        {usageData.usageByArea.map((area: any, index: number) => (
                          <div key={area.areaId} className="bg-white rounded-lg p-3 sm:p-4 border border-gray-200 max-w-full overflow-hidden">
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-full">
                              ...
                              <div className="text-center">
                                <p className="text-lg sm:text-2xl font-bold text-green-600">{area.recurringReservations}</p>
                                <p className="text-xs sm:text-sm text-gray-500">Recorrentes</p>
                              </div>
                              <div className="text-center">
                                <p className="text-lg sm:text-2xl font-bold text-btg-blue-deep">{area.usagePercentage}%</p>
                                <p className="text-xs sm:text-sm text-gray-500">Ocupação</p>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  {usageData?.summary && (
                    <div className="bg-white rounded-lg p-4 sm:p-6 border border-gray-200 max-w-full overflow-hidden">
                      ...
                      <div className="flex flex-row gap-4 sm:gap-6 max-w-full">
                        ...
                        <div className="text-center flex-1">
                          <p className="text-2xl sm:text-3xl font-bold text-blue-600">{usageData.summary.totalIndividual}</p>
                          <p className="text-xs sm:text-sm text-gray-500">Reservas Individuais</p>
                        </div>
                        <div className="text-center flex-1">
                          <p className="text-2xl sm:text-3xl font-bold text-green-600">{usageData.summary.totalRecurring}</p>
                          <p className="text-xs sm:text-sm text-gray-500">Reservas Recorrentes</p>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          {activeTab === 'export' && (
            <div className="space-y-4 sm:space-y-6">
              <div className="bg-gray-50 rounded-lg p-4 sm:p-6 border border-gray-200">
                <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-4 sm:mb-6">Exportar Dados</h3>
                
                <div className="flex justify-center">
                  <button 
                    onClick={handleExport}
                    disabled={!dateRange.startDate || !dateRange.endDate || dateRange.endDate < dateRange.startDate}
                    className="p-3 sm:p-4 bg-white rounded-lg border border-gray-200 hover:border-btg-blue-bright hover:bg-btg-blue-light hover:bg-opacity-10 transition-colors text-left group max-w-md w-full disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:border-gray-200 disabled:hover:bg-white"
                  >
                    <div className="flex items-center space-x-2 sm:space-x-3">
